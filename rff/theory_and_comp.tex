...
\end{equation}
\cite{NIPS2007_013a006f}*{page 3}. One caveat in Bochner's theorem is that it requires our kernel to be shift-invariant (sometimes also referred to as stationary) as stated in \Cref{defe: shift-invar}.
...
Clearly, the Gaussian RBF kernel is shift-invariant since it only relies on the bounding radius of $\bm{x}$ and $\bm{y}$. Thus, from Bochner's theorem, a positive definite shift-invariant kernel with $k(\bm{0}) = 1$ can be computed as
...
\end{equation}
The main idea of RFF is to approximate the integral in  \Cref{eq: rff_boch_int} using the following Monte-Carlo estimate
\begin{align*}
...
\end{equation}
which satisfies  \Cref{eq: rff_boch_int} since
\begin{align*}
...
\end{equation}
where $b \sim U \left[ 0,2 \pi \right]$. Using a similar argument we can show that this embedding also satisfies  \Cref{eq: rff_boch_int}. However, Sutherland and Schneider \cite{sutherland2015error} argue that the Gaussian RBF kernel is better suited for the embedding given in  \Cref{eq: rff_emb}. To summarise their argument we denote
...
\end{equation}
% TODO: fix equation!!!
to be the feature map corresponding to embedding in equation  \Cref{eq: rff_emb} and
\begin{equation} \label{eq: rff_feat_map_2}
    ...
    \end{bmatrix}
\end{equation}
to be the feature map corresponding to equation  \Cref{eq: rff_emb_alt}. They then show that
\begin{align*}
...
Rahimi and Recht prove \Cref{prop: rff_sup-norm_bound_1} for $\alpha = 2^8$ although Sutherland and Schneider improve this to $\alpha = 66$ \cite{sutherland2015error}*{page 3}. Observe that this bound is somewhat determined by the ratio $D/d$ which is why $D$ is often chosen as a multiple of $d$.

These results justify the RFF procedure seen in \Cref{alg: RFF-algorithm}, which was used to approximate a Gram matrix for the data set $\bm{X}$ using the feature map from  \Cref{eq: rff_feat_map_1}.
...
\begin{minipage}{.85\linewidth}
    \begin{algorithm}[H]
        ...
        \Output{$\bm{\widetilde{K}} \simeq \bm{K}$ where $\bm{K}$ is the Gram matrix corresponding to $\bm{X}$.}
        \BlankLine
...
\Cref{alg: RFF-algorithm} of course assumes an appropriate construction of $\bm{W}$, commonly called the transformation matrix, and thus has access to a routine which allows one to sample from $p(\cdot)$. When using the RBF Gaussian kernel, the spectral distribution given in equation  \Cref{eq: rbf-spec-pdf} corresponds to a multivariate Gaussian distribution with mean $\bm{0}$ and covariance matrix $\left( \frac{\sigma}{\sqrt{2}} \right)^{-2} \Id_{D \times D}$. This means $\bm{W}$ can simply be constructed as $\bm{W} = \left( \frac{\sigma}{\sqrt{2}} \right)^{-1} \left[ \bm{\omega}_1 ,\ldots , \bm{\omega}_D \right]^{\intercal}$ where $\; \bm{\omega}_i \stackrel{\text{iid}}{\sim} \calN \left( \bm{0} , \Id_{D \times D} \right)$. Tranformations matrices constructed in this manner are given the notation $\bm{W}_{\text{RFF}}$. It can be shown that if $\bm{W}_{\text{RFF}}$ is used as the transformation matrix in \Cref{alg: RFF-algorithm} then it produces an unbiased estimate, $\bm{\widetilde{K}}_{\text{RFF}}$, for the Gram matrix. This stated more precisely in \Cref{lem: rff-unbiased}.
