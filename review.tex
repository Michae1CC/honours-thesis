...
To keep this simple to begin with, we shall assume that our obsevered data $D = \left\{ (\bm{x_i}, \bm{f}_i) \mid i \in \left\{ 1,2, \ldots , n \right\} \right\}$ is (unrealistically) noise-free.
In other words, for any sample in our dataset we can be certain that the observed value is the true value of the underlying function we wish to model.
The joint distribution of the training points $\bm{f}$ and the test points $\bm{f}_{\ast}$ according to the prior is
\[
	\begin{pmatrix}
		\bm{f} \\
		\bm{f}_{\ast}
	...
	\end{pmatrix}
\]
where $\bm{K_{XX}} = k(\bm{X}, \bm{X}) \in \RR^{n \times n}$, $\bm{K_{XX^{\ast}}} = k(\bm{X}, \bm{X^{\ast}}) \in \RR^{n \times n_\ast}$, $\bm{K_{X^{\ast}X^{\ast}}} = k(\bm{X^{\ast}}, \bm{X^{\ast}}) \in \RR^{n_\ast \times n_\ast}$.

We can derive a distribution for $\bm{f}_{\ast}$ by conditioning the joint Gaussian prior distribution on the observed values
\begin{equation}\label{prop:GP_train_distr1}
	\bm{f}_{\ast} \mid \bm{K_{XX^{\ast}}} , \bm{K_{XX}}, \bm{f} \sim \calN \left( \bm{K_{XX^{\ast}}}^{\intercal} \bm{K_{XX}}^{-1},  \bm{K_{X^{\ast}X^{\ast}}} - \bm{K_{XX^{\ast}}}^{\intercal} \bm{K_{XX}}^{-1} \bm{K_{XX^{\ast}}}  \right)
\end{equation}
