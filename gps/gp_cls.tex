...
\end{equation}
will take the role of the sigmoid function in \Cref{eq: GPC-lin-model-1}, graphed in \Cref{fig: logistic-func-and-probit}. This type of model is aptly named the logistic regression.
\begin{figure}[ht]
    \centering
    \begin{tikzpicture}[>=latex, scale=1.1]
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{The logistic function from \Cref{eq: logistic-function} (solid red) juxtaposed with a close approximation, the scaled probit function (dashed blue).}
...
\begin{equation*}
    p \left( y \mid \bm{x} , \bm{w} \right) = \sigma \left( \langle \bm{x} , \bm{w} \rangle \right)^{\frac{1+y}{2}} \left( 1 - \sigma \left( \langle \bm{x} , \bm{w} \rangle \right) \right)^{\frac{1 - y}{2}}
\end{equation*}
...
\end{equation*}
This construction is illustrated in \Cref{fig: latent-func-and-sig-trans} and provides a natural extension to the linear logistic regression model.
...
\end{figure}
Specifically, the linear model from \Cref{eq: GPC-lin-latent-func} is replaced with a GPR model and the Gaussian prior on the weights with a GPR weight prior with
\begin{equation*}
    p \left(
    \begin{bmatrix}
            \bm{f} \\
...
\end{equation}
While this is a sound model, computing predictions is not so straight forward since the integral in  \Cref{eq: GP-pred-1} is not analytically tractable as applying $\sigma$ to $f_{\star}$ no longer produces a Gaussian distribution. Later on we will see how we can make use of our numerical toolbox to derive a good approximation for $\overline{\pi_{\star}}$.

\subsubsection{Lapace Approximation for Posterior}\label{Section1.6.2}

We saw that the integral in  \Cref{eq: GP-pred-1} could not be used to make predictions for $\overline{\pi_{\star}}$ analytically. In this section we shall address how the distribution for the latent process, $p \left( f_{\star} \mid \bm{X} , \bm{y} , \bm{x}_{\star} \right)$, can be approximated to provide a numerically tractable succedaneum. Using Baye's theorem
\begin{align*}
...
\end{align*}
using the fact that $p \left( \bm{y} \mid \bm{X} , \bm{x}_{\star}, \bm{f}, f_{\star} \right) = p \left( \bm{y} \mid \bm{X} , \bm{x}_{\star}, \bm{f} \right)$ \cites{BishopChristopherM2006Pram, RasmussenCarlEdward2006Gpfm}. The conditional distribution $p \left( f_{\star} \mid \bm{X} , \bm{x}_{\star}, \bm{f} \right)$ can be derived as
\begin{equation*}
...
\end{equation*}
through the use of \Cref{eq: GP_train_distr2_mean} and \Cref{eq: GP_train_distr2_var}. Unfortunately
\begin{equation*}
...
\end{equation*}
Once a suitable mode is found, using \Cref{eq: lapace-gauss-apprx}, the Lapacian approximation for $p \left( \bm{f} \mid \bm{X} , \bm{y} \right)$ becomes
...
With the Lapace approximation for $p \left( \bm{f} \mid \bm{X} , \bm{y} \right)$ (\Cref{eq: lapace-apprx}) and an exact probability distribution for $p \left( f_{\star} \mid \bm{X} , \bm{x}_{\star}, \bm{f} \right)$, a mean for the latent process, $p \left( f_{\star} \mid \bm{X} , \bm{y} , \bm{x}_{\star} \right)$, can now be computed by invoking  \Cref{eq: GP_train_distr2_mean} to give
\begin{align}
...
\end{align}
Similarly, the variance can be computed using \Cref{eq: GP_train_distr2_var} to give
...
\end{align}
Using \Cref{eq: GP-pred-1}, predictions can now be made as
...
\end{equation}
Again apply the approximation $\sigma \left( a \right) \simeq \Phi \left( \lambda a \right)$ to left hand side of  \Cref{eq: probit-int-apprx-1} gives the following estimate for the convolution of a Gaussian and sigmoid function
...
\end{equation}
\cite{BishopChristopherM2006Pram}*{page 219}. The integral used to approximate $\overline{\pi_{\star}}$ in  \Cref{eq: pred-apprx-1} can now be estimated using  \Cref{eq: pred-apprx-2} to give
