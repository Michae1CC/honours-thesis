...
\input{review/qr_and_gs.tex}

\input{review/arn_and lancz.tex}

\input{review/opt_conds.tex}

\input{review/conj_grad.tex}
...
Thus finding a mean an covariance for $\bm{f}_{\ast}$ requires a direct application of \Cref{theorem: cond_of_MVN} which gives
\begin{align*}
...
The proof of \Cref{lemma: W_left_ideal} can be adapted to show that $\calH$ is a two-sided ideal in $(\Fun(G),\star)$.
...
\]
\Cref{lemma: W_left_ideal} tells us that $w \star\alpha$ is indeed an element of $W$ so the image of $\Psi$ is indeed $\End(W)$.
...
We apply this to the induced representation $W$ and \Cref{prop: H_iso_End_G(W)} lets us conclude that $W$ is multiplicity-free if and only if $\calH$ is commutative.
...
\begin{proof}
    \begin{enumerate}[\itshape(i)]
        \item Apply \Cref{prop: V_mult-free_iff_End_G(V)_commutative} with $V=W$.
              Then $W$ is multiplicity-free if and only if $\End_G(W)$ is commutative.
              \Cref{prop: H_iso_End_G(W)} tells us that $\End_G(W)\cong\calH$.
...
\end{proof}
We employ \Cref{lemma: subalgebra_commutative} by applying it to the case where $A=\Fun(G)$ and $B=\calH(G,K)$. Recall from \Cref{Section1.3} that the characteristic functions $\{\chi_x\}_{x\in G}$ form a basis of $\calH(G,K)$.
...
\begin{proof}
    ...
    \]
\end{proof}
We are now ready to prove \Cref{theorem: Gelfand's_Trick}.
\begin{proof}[Proof of \Cref{theorem: Gelfand's_Trick}]
    \Cref{lem: id_on_basis} tells us that $\varphi^\ast$ is the identity on the characteristic functions $\chi_x$.
...
To be a Gelfand pair, it is sufficient to find an anti-automorphism satisfying the conditions of \Cref{theorem: Gelfand's_Trick}.
